...
public class DependencyConLL implements Serializable {
    ...
    private double id; // Word index, integer starting at 1 for each new sentence; may be a range for multiword tokens; may be a decimal number for empty nodes.
    ...
    private String feats; //List of morphological features from the universal feature inventory or from a defined language-specific extension; underscore if not available
    private double head; //Head of the current word, which is either a value of ID or zero (0)
    private String deprel; //Universal dependency relation to the HEAD (root iff HEAD = 0) or a defined language-specific subtype of one
    ...
    public boolean equals(Object obj){
        if (obj instanceof DependencyConLL) {
            ...
            if ( dependencyConLL.getLemma().equals(this.getLemma()) //&& dependencyConLL.getId()== this.getId()
                    && dependencyConLL.getXPosTag().equals(this.getXPosTag()) && dependencyConLL.getHead() == this.getHead()
                    && dependencyConLL.getDeprel().equals(this.getDeprel())){
                return true;
            } else {
                return false;
            }
