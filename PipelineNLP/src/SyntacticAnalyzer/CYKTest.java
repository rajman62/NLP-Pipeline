...
public class CYKTest {
     ...
     * OUTPUT FILES
     */
    private static String OUTPUT_DEPENDENCIES_STREAM = SYNTAX_INPUT_PATH_FOLDER +"udc_dependenciesCYK1stPass_TEST_NEW.ser";
    ...
    public ArrayList<GrammaticalRule> addToGrammar(ArrayList<GrammaticalRule> grammar, ArrayList<ArrayList<DependencyConLL>> parsedDependenciesConLL, int k){
        ...
        if (grammaticalRules!=null) {
                        ...
                        if (Double.compare(right.get(r).getId(),left.getId())==0){
                            ...
                            if (flag==true){
                                ...
                                if (first) {
                                    ...
                                    if (!grammar.contains(grammaticalRule)) {
                                        grammar.add(grammaticalRule);
                                    }
                                } else {
            ...
            for (int i = 0; i < grammar.size(); i++) {
                String pos = grammar.get(i).getLeftHandSide().getXpostag();
                String deprel = grammar.get(i).getLeftHandSide().getDeprel();
                ArrayList<String> deps;
                if (!tagsDeprels.containsKey(pos)) {
                    deps = new ArrayList<String>();
                    deps.add(deprel);
                } else {
                    deps = tagsDeprels.get(pos);
                    if (!deps.contains(deprel)) {
                        deps.add(deprel);
                        tagsDeprels.put(pos, deps);
                    }
                }

                for (int j = 0; j < grammar.get(i).getRightHandSide().size(); j++) {
                    pos = grammar.get(i).getRightHandSide().get(j).getXpostag();
                    deprel = grammar.get(i).getRightHandSide().get(j).getDeprel();
                    if (!tagsDeprels.containsKey(pos)) {
                        deps = new ArrayList<String>();
                        deps.add(deprel);
                    } else {
    ...
    public boolean checkExistenceofTrueChart (ArrayList<NonTerminal> [][] parsingChart, ArrayList<DependencyConLL> parsedDependenciesConLL) {
        ...
        if (parsedDependenciesConLL.size()==visitedDependencies.size()) {
            exists = true;
        }
        return exists;
    }
    ...
    public static void main(String[] args) {
        ...
        try{
            ...
            CYKTest cyk = new CYKTest();

            /**
             * 1.1. Loading the grammar from stream file
             */
            System.out.println("Load the grammar file");
            FileInputStream in = new FileInputStream(PATH_INPUT_GRAMMAR);
            ...
            in = new FileInputStream(PATH_TRAIN_DEPENDENCIES);
            stream = new ObjectInputStream(in);
            ...
            for (int ran = 0;ran<90;ran++) {
                //int Result = r.nextInt(High - Low) + Low;
                grammar = cyk.addToGrammar(grammar,trueDependencies,indices.get(ran));
            }
            ...
            int progress =0;
            for (int index: indices) { //chartsInfo.size()
                progress++;
                        ...
                        for (String key: chartsInfo.get(index).get(sub).get(subsub).keySet()) {
                            for(int subsubsub = 0; subsubsub<chartsInfo.get(index).get(sub).get(subsub).get(key).size();subsubsub++){
                                ...
                                if (parts.length>=2){
                                    ...
                                    //System.out.println(parts[0]+" "+emorTag);
                                    String pennTag;
                                    ...
                                    if(cyk.tagsDeprels.containsKey(pennTag)) {
                                        for (String deprel : cyk.tagsDeprels.get(pennTag)) {
                                            parsingChart[sub][subsub].add(new NonTerminal(subsub,parts[0], pennTag, deprel, -1, -1, -1));
                                        }
                                    }
                                }
                            }
            ...
            /*
            fos = new FileOutputStream(SYNTAX_INPUT_PATH_FOLDER+"not_runned_test.ser");
