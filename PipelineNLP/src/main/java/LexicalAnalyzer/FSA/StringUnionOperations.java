...
final public class StringUnionOperations {
	 ...
	 */
	private static LexicalAnalyzer.FSA.State convert(State s,
													 IdentityHashMap<State, LexicalAnalyzer.FSA.State> visited) {
		LexicalAnalyzer.FSA.State converted = visited.get(s);
		if (converted != null)
			return converted;

		converted = new LexicalAnalyzer.FSA.State();
	 ...
	 */
	public static LexicalAnalyzer.FSA.State build(CharSequence[] input) {
		final StringUnionOperations builder = new StringUnionOperations(); 

		for (CharSequence chs : input)
			builder.add(chs);

		return convert(builder.complete(), new IdentityHashMap<State, LexicalAnalyzer.FSA.State>());
	}
